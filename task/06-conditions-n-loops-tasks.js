...
{
    let r = Math.sqrt(Math.pow((point.x-circle.center.x), 2)+Math.pow((point.y-circle.center.y), 2));
    if (r<circle.radius)
    {
        return true;
    }       
...
function findFirstSingleChar(str) 
{
    ...
    {
        if ((str.indexOf(str[i], i + 1) == -1) && (arr.indexOf(str[i]) == -1)) 
        {
          return str[i];
        }
        arr.push(str[i]);
...
function isCreditCardNumber(ccn) 
{
    ...
    let parity = ccn.length % 2;
    for (let i = 0; i < ccn.length - 1; i++) 
    {
...
function timespanToHumanString(startDate, endDate) 
{
    ...
    if (time <= 90) 
        return 'a minute ago';
    if (time <= 45 * min) 
        return Math.round((time - 0.001) / min) + ' minutes ago';
    if (time <= 90 * min) 
        return 'an hour ago';
    if (time <= 22 * hour) 
        return Math.round((time - 0.001) / hour) + ' hours ago';
    if (time <= 36 * hour) 
        return 'a day ago';
    if (time <= 25 * day) 
        return Math.round((time - 0.001) / day) + ' days ago';
    if (time <= 45 * day) 
        return 'a month ago';
    if (time <= 345 * day) 
...
function getCommonDirectoryPath(pathes) 
{
    ...
    pathes.forEach((path) => 
    {
        ...
        while (i < min_length && common_path[i] == path[i]) 
            i++;
...
function evaluateTicTacToePosition(position) 
{
    ...
    for (let i=0; i<=2; i++)
    {
        if ((position[i][0] == position[i][1])&&(position[i][1] == position[i][2])&&(position[i][2]))
        return position[i][0];
        if ((position[0][i] == position[1][i])&&(position[1][i] == position[2][i])&&(position[2][i]))
        return position[0][i];
    }
    return undefined;
}
